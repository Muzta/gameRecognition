...
class ResNet34:
	...
	def build(numChannels, imgRows, imgCols, numClasses, activation="relu", weightsPath=None):
		...
		def resnetBlock(input, filters, kernelSize, activation, strides):
			x = Conv2D(filters, kernelSize, padding="same", strides=strides)(input)
			...
			input = Conv2D(filters, 1, strides, "same")(input)
		...
		x = Conv2D(64, 5, padding="same", strides=2)(inputs)
		x = BatchNormalization()(x)
		...
		for i in range(3):
			if i == 0:
				x = resnetBlock(x, 64, 3, activation, 2)
			else:
		...
		for i in range(2):
			if i == 0:
				x = resnetBlock(x, 1024, 3, activation, 2)
			else:
				x = resnetBlock(x, 1024, 3, activation, 1)

		
		""" x = AveragePooling2D(pool_size = 13, padding="same")(x)"""
